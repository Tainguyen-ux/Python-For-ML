...
```
+---------------------+         Lệnh chạy code        +---------------------+
| Client (Trình duyệt)| ----------------------------> |  Server (Colab VM)  |
| Giao diện tương tác | <---------------------------- | Thực thi CPU/GPU/TPU|
+---------------------+      Kết quả (Text/Plot)      +---------------------+
```
...
├── data_raw/          <-- File dữ liệu gốc thô (CSV/Excel)
├── data_processed/    <-- Dữ liệu sạch sau khi dọn dẹp
├── notebooks/         <-- File code Colab (.ipynb) phân tích
└── reports_figures/   <-- Biểu đồ xuất bản phẩm chất lượng cao
```
...
Trong nghiên cứu này, chúng tôi tiến hành đánh giá hiệu năng khai thác của
phân vùng bến bãi thông qua hai thông số vận hành cốt lõi và một hàm mục
tiêu tối ưu hóa chi phí.

## 1. Các chỉ số hiệu suất cảng biển (KPIs)

### 1.1 Thời gian quay vòng tàu (Vessel Turnaround Time)
Thời gian quay vòng của tàu $T_{\text{turnaround}}$ đại diện cho tổng thời gian
tàu lưu lại tại cảng, được xác định bằng hiệu số giữa thời điểm tàu rời bến
($T_{\text{departure}}$) và thời điểm tàu cập cầu cảng ($T_{\text{arrival}}$):

$$T_{\text{turnaround}} = T_{\text{departure}} - T_{\text{arrival}}$$

### 1.2 Tỷ lệ lấp đầy bãi container (Yard Utilization Rate)
Tỷ lệ lấp đầy bãi yard ($U_{\text{yard}}$) thể hiện mức độ quá tải của bãi
chứa container tại thời điểm khảo sát, tính bằng tỷ lệ phần trăm giữa lượng
container hiện có ($N_{\text{current}}$) và sức chứa tối đa thiết kế của bãi ($C_{\text{max}}$):

$$U_{\text{yard}} = \left( \frac{N_{\text{current}}}{C_{\text{max}}} \right) \times 100\%$$

## 2. Hàm mục tiêu tối ưu hóa đường đi của cẩu khung (RTG)
Để giảm thiểu lượng khí thải carbon và chi phí năng lượng của cẩu bãi RTG
(Rubber Tyred Gantry), mô hình toán học tìm cách tối ưu tổng quãng đường di
chuyển dịch chuyển bãi bốc xếp:
...
Nghiên cứu này tập trung vào việc nâng cao hiệu quả khai thác tại cảng nước
sâu Tân Cảng - Cái Mép (TCIT). Chúng tôi đề xuất một khung phân tích dữ liệu
tích hợp để dự báo **thời gian lưu bãi (yard dwell time)** của container nhập
khẩu và tối ưu hóa **kế hoạch phân bổ cầu bến (berth allocation plan)**. Bằng
cách áp dụng các thuật toán máy học trên tập dữ liệu vận hành bãi cảng lịch
sử, nghiên cứu hướng tới giảm thiểu **thời gian chờ đợi của tàu** tại luồng
hàng hải và nâng cao hệ số sử dụng cẩu bờ STS. Kết quả mô phỏng dự kiến sẽ
đóng góp một giải pháp điều hành cảng thông minh, hỗ trợ giảm chi phí logistics
...
Dưới đây là bảng tổng hợp các thông số kỹ thuật hiện tại của cảng nước
sâu TCIT được sử dụng làm biến ràng buộc trong mô hình tối ưu hóa bốc xếp bãi:
...
Để đánh giá mức độ nghẽn cảng và tối ưu hóa chi phí di chuyển thiết bị nâng hạ,
chúng tôi thiết lập hai phương trình đánh giá toán học sau:

### 2.1 Chỉ số Thời gian làm hàng tại bến (Port Stay Time)
Tổng thời gian làm hàng thực tế của một tàu container tại cảng vụ ($T_{\text{stay}}$)
được xác định bằng hiệu số thời gian từ lúc tàu hoàn thành thủ tục rời bến
($T_{\text{dep}}$) tới khi tàu thả neo và cập bến thành công ($T_{\text{arr}}$):

$$T_{\text{stay}} = T_{\text{dep}} - T_{\text{arr}}$$

### 2.2 Hàm mục tiêu giảm thiểu chi phí chuyển đổi bãi yard (Container Shifting Cost)
Khi bãi yard bị quá tải ($U_{\text{yard}} > 80\%$), cẩu khung RTG phải thực hiện
nhiều thao tác đảo container (dịch chuyển không sinh công) để lấy được container
đích ở hàng dưới cùng. Chúng tôi biểu diễn hàm mục tiêu tối thiểu hóa chi phí
năng lượng đảo chuyển container như sau:

$$\min \Phi = \sum_{p \in P} \sum_{q \in Q} E_{pq} \cdot Y_{pq} \cdot C_{\text{power}}$$

Trong đó:
*   $P, Q$: Tập hợp các block lưu trữ container trong bãi yard.
*   $E_{pq}$: Mức tiêu thụ điện năng của cẩu RTG khi thực hiện đảo container từ
    vị trí $p$ sang vị trí phụ $q$ ($kWh$).
*   $Y_{pq}$: Biến quyết định nguyên, biểu thị số lần dịch chuyển container
    thực tế từ $p$ sang $q$.
...
* **Cách viết vào bài báo khoa học (Phần Data Source and Profiling):**
  > "Initial data profiling was performed on the gate transit registry containing simulated records of truck movements. The data schema contains numeric payloads (`Weight_Tons`) and nominal categoricals (`Gate_Action`, `Shift`). Diagnostic checks confirmed a high rate of vehicle recycling, with duplicate plate logs representing repeat terminal visits. Operational intensity was concentrated during the daytime shift (Day Shift), accounting for approximately 60.0% of the total gate transactions."
  >
...
```
[Ngưỡng dưới: Q1-1.5*IQR] <─── Dải hợp lệ ───> [Ngưỡng trên: Q3+1.5*IQR]
```
...
```
[Lịch Tàu: Vessel_ID|ETA] <──(pd.merge)──> [Hàng Hóa: Vessel_ID|Teu_Qty]
```
...
* **Cách viết vào bài báo khoa học (Phần Data Integration):**
  > "To link vessel spatial occupancy with handling requirements, relational joins were performed. The vessel berth planning database and the cargo declaration registry were integrated through an inner join on the unique voyage identification key (`Vessel_ID`). This procedure produced a consolidated training matrix mapping the ship's physical arrival window directly to its cargo handling capacity (TEUs), eliminating voyages with incomplete manifest profiles."
  >
...
```
 [TOS DB (Postgres)] ──(SQL Query)──> [DataFrame (Raw JSONB)]
                                             │
                                     (pd.json_normalize)
                                             ▼
                                    [Flat DF (ML Ready)]
...
```
 [Raw Log Files] ──(Try-Except Guarded ETL)──> [Clean & Standardized]
                                                      │
                                              (Automatic Export)
                                                      ▼
                                            [Data Warehouse/ML]
...
df_cleaned['Dwell_Time_Gio'] = (
    df_cleaned['Thoi_Gian_Ra_Cong'] - df_cleaned['Thoi_Gian_Vao_Bai']
...
cols_to_show = [
    'Ma_Container', 'Thoi_Gian_Vao_Bai', 
    'Dwell_Time_Gio', 'Gio_Vao_Cong', 'La_Cuoi_Tuan'
]
print("\n--- BẢNG DỮ LIỆU SAU KHI TIỀN XỬ LÝ THỜI GIAN ---")
print(df_cleaned[cols_to_show])
```
...
ais_logs = {
    ...
    'Latitude': [
        10.2312, 10.2450, 45.9999, 10.2510, np.nan, 10.2680, 10.2750
    ], # 45.9999 là nhiễu nhảy vọt GPS, np.nan là điểm khuyết
    'Longitude': [
        107.0121, 107.0250, -120.4422, 107.0310, 107.0380, np.nan, 107.0520
    ], # np.nan là điểm khuyết kinh độ
...
# Thay thế các điểm nhiễu bằng NaN để thuật toán nội suy lại
...
# Thực hiện nội suy tuyến tính (Linear Interpolate) dọc theo trục thời gian
...
# 1. Giả lập hoạt động bãi yard (Block Occupancy) và cổng Gate theo giờ
np.random.seed(42)
time_index = pd.date_range(
    start='2026-07-02 00:00:00', end='2026-07-02 23:00:00', freq='h'
)
n_hours = len(time_index)

# Lưu lượng xe tải qua cổng chính thức trong từng giờ
gate_sin = np.sin(np.arange(n_hours) * np.pi / 12)
gate_traffic = 30 + 20 * gate_sin + np.random.randint(-5, 5, n_hours)
gate_traffic = np.clip(gate_traffic, 5, None)  # Đảm bảo lưu lượng không âm

# Lượng container đang lưu trữ trong Yard Block A (Sức chứa tối đa: 1000 TEUs)
...
# window=3 thiết lập cửa sổ trượt 3 giờ, tránh NaN ở những giờ đầu
df_yard['Rolling_Gate_Traffic_3h'] = (
    df_yard['Gate_Traffic'].rolling(window=3, min_periods=1).mean()
)
...
cols_to_print = [
    'Timestamp', 'Gate_Traffic', 'Rolling_Gate_Traffic_3h',
    'Gate_Traffic_Delta', 'Yard_TEUs', 'Yard_Density_Pct'
]
print("--- KẾT QUẢ TRÍCH XUẤT ĐẶC TRƯNG LOGISTICS NÂNG CAO ---")
print(df_yard[cols_to_print].head(8))
```
...
berth_data = {
    'Vessel_Name': [
        'MSC Aurora', 'CMA CGM Jean', 'Maersk Clyde', 
        'ONE Continuity', 'Cosco Shipping'
    ],
...
def detect_conflicts(df):
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            # So sánh khoảng chênh lệch thời gian chồng lấn
            start_max = max(df.iloc[i]['Start_Time'], df.iloc[j]['Start_Time'])
            end_min = min(df.iloc[i]['End_Time'], df.iloc[j]['End_Time'])
            overlap = max(pd.Timedelta(0), end_min - start_max) > pd.Timedelta(0)
...
def plot_berth_gantt(selected_berth='All'):
    if selected_berth == 'All':
        ...
        is_sel = df_schedule['Berth_ID'] == selected_berth
        df_filtered = df_schedule[is_sel].copy()
    ...
    for idx, row in df_filtered.iterrows():
        ...
        if is_conflict:
            lbl += " [XUNG ĐỘT!]"
        ax.text(
            x=start_num + duration/2, y=y_pos, s=lbl,
    ...
    t_str = f"Bảng điều khiển Phân bổ Cầu bến ({selected_berth})"
    ax.set_title(t_str, fontsize=12, fontweight='bold', pad=15)
    ax.set_xlabel(
        "Mốc thời gian Vận hành (Tháng-Ngày Giờ:Phút)", 
...
```
[Vessels (Tàu)] ──(Sản lượng)──> [Yard Blocks] ──(Dòng xe)──> [Smart Gates]
```
...
node_labels = [
    "Tàu A (MSC)", "Tàu B (CMA)", 
    "Bãi chứa Block 01", "Bãi chứa Block 02", 
    "Smart Gate 01", "Smart Gate 02"
]
...
fig_sankey.update_layout(
    title_text="Động học Dòng chảy Container: Tàu -> Bãi -> Cổng",
    font_size=10,
    width=800,
    height=500
)
...
ax.set_zlabel('Chiều cao chồng (Tiers)', fontweight='bold', labelpad=8)
    t_str = 'Topology Phân bổ Xếp chồng Bãi 3D (3D Yard Block Stacking)'
    ax.set_title(t_str, fontsize=12, fontweight='bold', pad=15)
    ax.set_zlim(0, 6)
...
def greedy_yard_allocation(berth_id, qty_containers):
    ...
    best_dist = berth_distances[best_block_idx]
    best_block = block_names[best_block_idx]
    print(
        f"    * Chọn vị trí tối ưu: {best_block} "
        f"(Quãng đường ngắn nhất: {best_dist}m)"
    )
...
print(
    f">> Bước 2: Đã nạp thành công '{raw_data_file}'."
    f" Kích thước dữ liệu: {df_raw.shape}"
)
...
disp = ConfusionMatrixDisplay(
    ...
    display_labels=['Khoe_manh', 'Benh_ly']
)
